...
export function handleTransfer(event: Transfer): void {
  ...
  let pairContract = PairContract.bind(event.address) as PairContract;
  ...
  if (event.params.to.toHexString() == pair.id) {
    ...
    let burn = new BurnEvent(
      event.transaction.hash
        .toHexString()
        .concat("-")
        .concat(BigInt.fromI32(burns.length as i32).toString())
    );
...
export function handleSwap(event: Swap): void {
  ...
  let swaps = transaction.swaps as string[];
  let swap = new SwapEvent(
    event.transaction.hash
      .toHexString()
      .concat("-")
      .concat(BigInt.fromI32(swaps.length as i32).toString())
  );
